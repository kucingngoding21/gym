...
@section('content')
    <h3 class="text-dark mb-4">Edit Data Member</h3>
    @if ($errors->any())
    ...
    @endif
    <div class="p-2">
        <a href="{{ route('index.depositkelas') }}" class="btn btn-primary btn-sm">Kembali</a>
    </div>
    <div class="card shadow">
        <div class="card-header py-3">
            <div class="row">
                <div class="col">
                    <p class="text-primary m-0 fw-bold">Form Edit Member</p>
                </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('update.depositkelas', ['id' => $depositkelas->id]) }}"
                        ...
                        @foreach ($user as $user)
                            @if ($user->role_name === 'member')
                                <option value="{{ $user->id }}"
                                    {{ $depositkelas->id_member == $user->id ? 'selected' : '' }}>{{ $user->first_name }}
                                </option>
                            @endif
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="jenis_kelas" class="form-select" id="jenis_kelas">
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}" @if($k->id == $depositkelas->jenis_kelas) selected @endif>{{ $k->nama_kelas }}</option>
                        @endforeach
                    </select>
                    @error('jenis_kelas')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" readonly="" name="biaya" id="biaya" value="{{ $depositkelas->biaya_kelas }}" class="form-control"
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="jangka_waktu" value="{{ $depositkelas->jangka_waktu }}" class="form-control"
                ...
                <div class="mb-3">
                    ...
                    <input type="number" readonly name="nominal_deposit" id="nominal_deposit" value="{{ $depositkelas->nominal_deposit }}" class="form-control"
                           id="exampleInputEmail1" aria-describedby="emailHelp" oninput="formatRupiah(this)">
                </div>
                <div class="mb-3">
                    <label class="form-label">Masa Aktif</label>
                    <input type="text" name="expired" id="expired" value="{{ $depositkelas->expired }}"
...
{{--                    <input type="text" name="nominal_deposit" readonly value="{{ $depositkelas->nominal_deposit }}"--}}
...
{{--                    <input type="date" name="tanggal" value="{{ $depositkelas->tanggal }}" class="form-control"--}}
