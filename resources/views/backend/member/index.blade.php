        ...
        <div class="card-header py-3">
            <div class="row">
                ...
                <div class="col text-right">
                    <a href="{{ route('create.member') }}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Tambah Data</a>
                </div>
            </div>
        </div>
        <div class="card-body">
            ...
            <div class="table-responsive mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                <table class="table my-0" id="datatable-buttons">
                    ...
                    @foreach ($data as $d)
                        <tr>
                            <td>{{ $d->format_id_member }}</td>
                            <td>{{ $d->first_name }} {{ $d->middle_name }} {{ $d->last_name }}</td>
                            <td>{{ $d->alamat }}</td>
                            <td>{{ $d->email }}</td>
                            <td>{{ $d->no_telpon }}</td>
                            <td>
                                <a href="{{ route('show.member', $d->id) }}" title="Show Member">
                                    <i class="fas fa-eye" style="--bs-primary: #094067;--bs-primary-rgb: 9,64,103;"></i>
                                </a>
                                <a href="{{ route('edit.member', $d->id) }}" title="Edit Member"  style="padding-left: 10px;">
                                    <i class="fas fa-edit" style="--bs-primary: #094067;--bs-primary-rgb: 9,64,103;"></i>
                                </a>
                                <a href="{{ route('destroy.member', $d->id) }}" onclick="return confirm('are you sure?')" style="padding-left: 10px;" title="Hapus Member">
